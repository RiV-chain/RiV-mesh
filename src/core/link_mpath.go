...
func (l *linkMPATH) dial(ctx context.Context, url *url.URL, info linkInfo, options linkOptions) (net.Conn, error) {
	conn, err := l.connFor(url, info.sintf)
	if err != nil {
		return nil, err
	}
...
func (l *linkMPATH) listen(ctx context.Context, url *url.URL, sintf string) (net.Listener, error) {
	/*
		hostport := url.Host
		if sintf != "" {
