...
func (t *tcp) listenSctp(u *url.URL, _ tcptls) (*TcpListener, error) {
	...
	listener, err := sctp.NewSCTPListener(addr, sctp.InitMsg{NumOstreams: 1, MaxInstreams: 1}, sctp.OneToOne, false)
	...
	go func() {
		...
		if options.socksProxyAddr != "" {
			...
			case "sctp":
				...
				conn, err = sctp.NewSCTPConnection(addr.AddressFamily, sctp.InitMsg{NumOstreams: 1, MaxInstreams: 1}, sctp.OneToOne, false)
				if err != nil {
