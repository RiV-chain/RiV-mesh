...
type NodeConfig struct {
	...
	HttpsAddress        string                     `comment:"Listen address for admin rest requests and web interface. Default is to listen for local\nconnections on TCP/19319. To disable the admin rest interface,\nuse the value \"none\" instead. Example: https://localhost:19019."`
	CertFile            string                     `comment:"TLS certificates file name. Name should has absolute path or path relative to config file folder"`
	KeyFile             string                     `comment:"TLS certificates file name. Name should has absolute path or path relative to config file folder"`
