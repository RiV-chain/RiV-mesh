...
type NodeConfig struct {
	...
	DDnsServer          DnsServerConfig            `comment:"dDNS server config. ListenAddress is the address:port to listen on. In order to listen on the loopback interface only, use 127.0.0.1:53. To listen on any interface, use [::]:53 (default "[::]:53"). UpstreamServers for DNS-over-TLS forwarding. This is a list of upstream servers (default "one.one.one.one:853@1.1.1.1,dns.google:853@8.8.8.8")"`
