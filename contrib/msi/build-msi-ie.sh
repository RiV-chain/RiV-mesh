...
#Sign Mesh binaries
[ "${SIGN}" == "sign" ] &&  cmd /c signtool sign /tr http://timestamp.sectigo.com /td sha256 /fd sha256 /f "$CERT_PATH" /d "RiV-mesh app" /a mesh.exe meshctl.exe
...
if [[ "${SIGN}" == "sign" ]];
then
  cmd /c signtool sign /tr http://timestamp.sectigo.com /td sha256 /fd sha256 /f "$CERT_PATH" /d "RiV-mesh app" /a ${PKGNAME}-${PKGVERSION}-${PKGARCH}-win7-ie.msi
fi